...
(async () => {
  if (!fs.existsSync('screenshots/light-mode')) {
    fs.mkdirSync('screenshots/light-mode', { recursive: true });
  }
  ...
  await context.addInitScript(() => {
    localStorage.setItem('trai-theme', 'light');
    document.documentElement.classList.add('light-mode', 'bot-detected');
    window.addEventListener('DOMContentLoaded', () => {
        document.body.classList.add('light-mode', 'bot-detected');
  ...
  for (const p of pages) {
    try {
      ...
      const screenshotName = `screenshots/light-mode/${p.replace('.html', '.png')}`;
